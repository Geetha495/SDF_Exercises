...
19. Regarding visitor pattern discussed in the class. The exercise below are in no particular order and one can do mix and match:
  1. Fix the parsing and/or evaluation so that the answer expected for (3+4-5+6-7) = 1
  2. Make the example generic (either by making use of any generic or using templates for const_exprt) for taking different kinds of constants
  3. Enrich the input language by supporting other binary operators (mul_exprt, div_exprt, rem_exprt, shr_exprt (>>), shl_exprt(<<) etc)
  4. Add var_exprt with type info and separately implement contextt or symbol_tablet that can provide a value for variable
  5. Remove the need for dynamic_cast by augmenting interface of visitort for all expressions.
